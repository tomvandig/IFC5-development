...
export const Duration = Object.freeze({
  ...
  parseISO8601(duration: string, maxLength: number = 100): Duration {
    ...
    function parseFloatNormal(match: string | undefined): number {
      ...
      if (isNaN(parsed))
        throw new Error(
          `Unreachable: Invalid number in ISO8601 duration string: ${match}`,
        );
  ...
  toISO8601(duration: Duration): string {
    ...
    const years =
      duration.years !== 0 && !isNaN(Number(duration.years))
        ? `${duration.years}Y`
        : "";
    const months =
      duration.months !== 0 && !isNaN(Number(duration.months))
        ? `${duration.months}M`
        : "";
    const weeks =
      duration.weeks !== 0 && !isNaN(Number(duration.weeks))
        ? `${duration.weeks}W`
        : "";
    const days =
      duration.days !== 0 && !isNaN(Number(duration.days))
        ? `${duration.days}D`
        : "";
