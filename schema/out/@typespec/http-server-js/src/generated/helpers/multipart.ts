...
function MultipartBoundaryTransformStream(
  ...
  let _readableController: ReadableStreamDefaultController<
  ...
  const writable = new WritableStream<Buffer>({
    write: async (chunk) => {
      ...
      while ((index = buffer.indexOf(boundarySplit)) !== -1) {
        ...
        if (index > 0) {
          if (!initialized) {
            if (
              !buffer.subarray(index - 2, index).equals(Buffer.from("\r\n"))
            ) {
              readableController.error(
                new Error("Invalid preamble in multipart body."),
              );
            } else {
    ...
    close() {
      if (!/--(\r\n)?/.test(buffer.toString("utf-8"))) {
        readableController.error(
          new Error("Unexpected characters after final boundary."),
        );
      }
  ...
  async function enqueueSub(s: Buffer) {
    subController ??= await new Promise<ReadableStreamDefaultController>(
      (resolve) => {
 ...
 */
export function createMultipartReadable(
  request: http.IncomingMessage,
): ReadableStream<HttpPart> {
